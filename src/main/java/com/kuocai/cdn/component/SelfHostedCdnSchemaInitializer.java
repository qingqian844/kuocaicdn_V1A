...
public class SelfHostedCdnSchemaInitializer {
    ...
    public void init() {
        ...
        execute("CREATE TABLE IF NOT EXISTS self_hosted_node_group (" +
                "id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,group_name VARCHAR(128) NOT NULL,cname_label VARCHAR(64) NOT NULL,dns_record_ids LONGTEXT NULL," +
        ...
        execute("INSERT INTO self_hosted_node_group (group_name,cname_label,is_default,status) " +
                "SELECT '默认节点组','edge',1,'enabled' WHERE NOT EXISTS (SELECT 1 FROM self_hosted_node_group WHERE is_default=1)");
    }
